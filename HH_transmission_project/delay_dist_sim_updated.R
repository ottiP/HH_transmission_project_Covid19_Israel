...
delay_dist_sim_updated_age <- function(df1){
  n.sim <- nrow(df1)
  ...
  df1$first.date.hh <- first.day.hh

  df1$date.exposed[df1$infected==0] <- df1$first.date.hh
  ...
  df1$max.time.hh <- as.numeric(round(difftime(enddate,df1$first.date.hh,units="days")))+1
  return(df1)
}
